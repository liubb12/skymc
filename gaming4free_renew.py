...
# -*- coding: utf-8 -*-
# ============================================================
# Gaming4Free 自动续期与开关机巡检 (激励广告等待与自动关弹窗版)
# ============================================================
import atexit
import base64
import html
...
def wait_and_dismiss_reward_ad(driver, max_wait=25):
    ...
    while time.time() - start_time < max_wait:
        ...
        # 之前发现了广告，现在 'until reward' 消失了，说明奖励已到账，立刻点击右上角 ✕ 关掉它
        if ad_detected:
        ...
        # 如果前 5 秒内根本没出现广告字样，说明本次无需看视频，直接跳过
...
def do_renew_and_start(driver):
    ...
    if valid_free_btn:
        ...
        if btn_disabled:
            ...
            # 4. 关键：等待激励视频广告播放完毕并自动关闭弹窗
            wait_and_dismiss_reward_ad(driver, max_wait=25)

            # 5. 如果伴随 Cloudflare Turnstile 验证则辅助通过
    ...
    if sec_after - sec_before >= 3000:
        ...
        if "冷却中" not in action_desc and "禁用" not in action_desc:
            action_desc = "⚠️ 倒计时未增加（可能处于冷却或广告未计入）"
    else:
        if renew_executed:
